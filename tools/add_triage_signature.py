...
class MustGatherAnalysis(Signature):
    ...
    def _update_ticket(self, url, issue_key, should_update=False):
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar.gz", delete=False) as tmp_mustgather:
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(prefix="insights-report-", suffix=".txt") as tmp_report:
                    try:
                        ...
                        logger.error(f"Error while writing and uploading {tmp_report.name}")
            except:
                logger.error(f"Error while handling must-gather in {tmp_mustgather.name}")
